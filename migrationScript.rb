...
res.each do |row|
    ...
    clipre.exec("INSERT INTO Coursesothers(classid,name,point,area,teacher,climit,ctype,note) VALUES('"+row["id"]+"','"+row["name"]+"',"+row["point"].to_s+",'"+row["area"]+"','"+row["teacher"]+"','"+row["climit"].to_s+"','"+row["ctype"].to_s+"','"+note+"')")
end
